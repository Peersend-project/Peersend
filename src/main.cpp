...
uint256 hashGenesisBlock("0x366e114b7b7052dbb129e59ead2fa4dcf91cd8dac8e23cdf7042820b735699da");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Peersend: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Peersend Signed Message:\n";
...
{
    // Peersend: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 84000000); // Peersend: 84m blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Peersend: 2 days
static const int64 nTargetSpacing = 2 * 60; // Peersend: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Peersend: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Peersend: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6d8f66233b4adf25ec0df270b330d6a847ec22cecfcfb1bc80d44e47f3a2fe57");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Mars Curiosity rover takes selfie with 'Mont Mercou' 30/03/2021";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048e160db6283bf3d515f55ea73c74a4d3e003c78e64ab4da86858c8ad812143975f484437dd6018dc9b8da5ef00436c9d281e078fafb67e633f27f9f9e4589b68") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1617142058;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085912388;

        if (fTestNet)
        {
            block.nTime    = 1617142024;
            block.nNonce   = 388676324;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x247dcdf8274085af45d114dbdd5ff54f4d00b9bfb8c5069a72198902b3bb6c0e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf8, 0xc5, 0xb9, 0xde }; // Peersend: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PeersendMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PeersendMiner(CWallet *pwallet)
{
    printf("PeersendMiner started\n");
    ...
    try { loop {
        ...
        printf("Running PeersendMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PeersendMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PeersendMiner, pwallet));
}
